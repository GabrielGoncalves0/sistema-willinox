...
export default function ProducaoTable({
  ...
  const filteredProducoes = producoes.filter((producao) => {
    // Adicione aqui lógica de filtro se desejar implementar busca
    // Exemplo: return producao.produto.nome.toLowerCase().includes(searchTerm.toLowerCase());
    if (!searchTerm) return true;
    const funcionario = producao?.fisica?.pessoa?.nome?.toLowerCase() || '';
    const produto = producao?.produto?.nome?.toLowerCase() || '';
    const status = formatStatusText(producao.status).toLowerCase();
    return (
      funcionario.includes(searchTerm.toLowerCase()) ||
      produto.includes(searchTerm.toLowerCase()) ||
